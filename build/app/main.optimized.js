  ...
  List: [
    {
      ...
      isAvailable: function(adapter, name) {
        console.log(adapter, name);
        return adapter === 'phone';
      }
    },
...
define('devicedetect',['implementations'], function (implementations) {
  ...
  var devicedetect = {
    ...
    load: function (name, req, onLoad, config) {
      ...
      if (hasMultipleImpls) {
        ...
        for (i = 0, m = featureInfo.length; i < m; i++) {
          ...
          if (current.isAvailable(config.adapter)) {
            if (typeof current.module !== 'undefined') {
              onLoad(current.module());
              return;
            }
      ...
      //by the module name + extension, but do not include the
      //!strip part to avoid file system issues.
      devicedetect.get(toLoad + '.js', function (content) {
        devicedetect.finishLoad(name, content, onLoad, config);
      });
    },
    ...
    writeFile: function (pluginName, moduleName, req, write, config) {
      var parsed = devicedetect.parseName(moduleName),
          nonStripName = parsed.moduleName + '.' + parsed.ext,
          //Use a '.js' file name so that it indicates it is a
          //script that can be loaded across domains.
          fileName = req.toUrl(parsed.moduleName + '.' +
      ...
      //write out values that do not have the strip argument,
      //to avoid any potential issues with ! in file names.
      devicedetect.load(nonStripName, req, function (value) {
        //Use own write() method to construct full module value.
        //But need to create shell that translates writeFile's
        //write() to the right interface.
        var textWrite = function (contents) {
        ...
        devicedetect.write(pluginName, nonStripName, textWrite, config);
      }, config);
    },
  ...
  if (devicedetect.createXhr()) {
    devicedetect.get = function (url, callback) {
      var xhr = devicedetect.createXhr();
      xhr.open('GET', url, true);
      xhr.onreadystatechange = function (evt) {
        //Do not explicitly handle errors, those should be
        //visible via console output in the browser.
...
require.config({
  paths: {
    ...
    // This plugin allows for a conditional require. Requiring a file with this pluggin triggers
    // a succession of small arbitrary tests which refine which file should actually be required.
    'devicedetect'    : 'js/lib/devicedetect'
  }
});
